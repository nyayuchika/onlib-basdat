...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="admin.php" title="Go to Dashboard" class="tip-bottom"><i class="icon-home"></i>Dashboard</a> <a href="dataadmin.php" class="current">Data Admin</a> </div>
    <h1>Ubah</h1>
  </div>
  <?php
	include "config.php";
	if(isset($_POST['kirim']))
	{
		...
		$sql="UPDATE admin SET username='$username', password='$password', level='$level' WHERE username='$username'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/superadmin.php/dataadmin.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/superadmin/ubahadmin.php'; </script>";
		}
	}
	?>
	<?php
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahadmin.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>Username :</label>
              <div class='controls'>
                <input type='text' name='username' class='span11'  value='".$data['username']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Password :</label>
              <div class='controls'>
                <input type='text' name='password' class='span11'  value='".$data['password']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Level :</label>
              <div class='controls'>
                <input type='text' name='level' class='span11'  value='".$data['level']."'/>
              </div>
            </div>
