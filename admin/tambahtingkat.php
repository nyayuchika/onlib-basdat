        ...
        <div class="widget-content nopadding">
          <form method="post" class="form-horizontal">
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="id_tingkat" class="span11"/>
              </div>
            </div>
            <label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM rak";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_rak'];?>">
					<?php echo $data['nama_rak'];?></option>
					<?php } } ?>
			</select>
			</div>
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="nama_tingkat" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO tingkat(id_tingkat,id_rak,nama_tingkat) VALUES('$_POST[id_tingkat]','$_POST[id_rak]','$_POST[nama_tingkat]')");
				if($sql)
				{
					echo "<script>alert('Data berhasil ditambahkan'); window.location = 'tingkat.php'</script>";
				}
				else
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahtingkat.php'</script>";
				}
				}
			?>
          </form>
