      ...
      <div class="span12">
        <div class="widget-box">
		  ...
		  <div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="no_pinjam" class="span11"/>
              </div>
            </div>
			<label class="control-label">
              Kartu :</label>
			  <div class="controls">
              <select class="form-control" name="no_kartu" required>
			  <option value="">Pilih Kartu</option>
			  <?php $sql="SELECT * FROM kartu JOIN mahasiswa ON kartu.nim=mahasiswa.nim";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_kartu'];?>">
					<?php echo $data['nama_mahasiswa'];?></option>
					<?php } } ?>
			</select>
			</div>
			<label class="control-label">
			...
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="jumlah_pinjam" class="span11"/>
              </div>
            </div>
			<div class="control-group">
              <label class="control-label">Denda :</label>
              <div class="controls">
                <input type="text" name="denda" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO peminjaman(no_pinjam,no_kartu,id_buku,jumlah_pinjam,denda) VALUES('$_POST[no_pinjam]','$_POST[no_kartu]','$_POST[id_buku]','$_POST[jumlah_pinjam]','$_POST[denda]')");
				if($sql)
				{
				...
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahpeminjaman.php'</script>";
				}
				}
			?>
          </form>
