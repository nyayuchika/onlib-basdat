...
<div id="sidebar"><a href="admin.php" class="visible-phone"><i class="icon icon-home"></i>Dashboard</a>
  <ul>
	...
	<li class="submenu"> <a href="#"><i class="icon icon-th-list"></i> <span>Fakultas & Jurusan</span> <span class="label label-important">2</span></a>
      <ul>
        <li class="active"> <a href="fakultas.php"><span>Fakultas</span></a> </li>
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="admin.php" title="Go to Dashboard" class="tip-bottom"><i class="icon-home"></i>Dashboard</a> <a href="fakultas.php" class="current">Fakultas</a> </div>
    <h1>Ubah</h1>
  </div>
  <?php
	include "config.php";
	if(isset($_POST['kirim']))
	{
		...
		$sql="UPDATE fakultas SET id_fakultas='$id_fakultas', nama_fakultas='$nama_fakultas' WHERE id_fakultas='$id_fakultas'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/fakultas.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/fakultas.php'; </script>";
		}
	}
	?>
	<?php
	$id_fakultas=isset($_GET['id_fakultas']) ? $_GET['id_fakultas']:'';
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahfakultas.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Fakultas :</label>
              <div class='controls'>
                <input type='text' name='id_fakultas' class='span11'  value='".$data['id_fakultas']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Nama Fakultas :</label>
              <div class='controls'>
                <input type='text' name='nama_fakultas' class='span11'  value='".$data['nama_fakultas']."'/>
              </div>
            </div>
            <div class='form-actions'>
              <button class='btn btn-primary' name='kirim'>Ubah</button>
			  <a href='fakultas.php' class='btn btn-warning'> Kembali </a>
            </div>
          </form>";
		  }
