...
<div id="sidebar"><a href="admin.php" class="visible-phone"><i class="icon icon-home"></i>Dashboard</a>
  <ul>
	...
	<li class="submenu"><a href="#"><i class="icon icon-th-list"></i> <span>Data Pengarang</span><span class="label label-important">2</i></a>
		...
		<li class="active"><a href="mengarang.php"><span>Pengarang</span></a></li>
		</ul>
	<li>
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="admin.php" title="Go to Dashboard" class="tip-bottom"><i class="icon-home"></i>Dashboard</a> <a href="mengarang.php" class="current">Pengarang</a> </div>
    <h1>Ubah</h1>
  </div>
  <?php
	include "config.php";
	...
	{
		$id_mengarang=isset($_POST['id_mengarang']) ? $_POST['id_mengarang']:'';
		$id_buku=isset($_POST['id_buku']) ? $_POST['id_buku']:'';
		$id_pengarang=isset($_POST['id_pengarang']) ? $_POST['id_pengarang']:'';
		$nama_pengarang=isset($_POST['nama_pengarang']) ? $_POST['nama_pengarang']:'';
		$sql="UPDATE mengarang SET id_mengarang='$id_mengarang', id_buku='$id_buku', id_pengarang='$id_pengarang', nama_pengarang='$nama_pengarang' WHERE id_mengarang='$id_mengarang'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/mengarang.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/mengarang.php'; </script>";
		}
	}
	?>
	<?php
	$id_mengarang=isset($_GET['id_mengarang']) ? $_GET['id_mengarang']:'';
	$sql="SELECT * FROM mengarang WHERE id_mengarang='$id_mengarang'";
	$hasil=mysqli_query($koneksi, $sql);
    echo "
  <div class='container-fluid'>
    ...
    <div class='span12'>
      <div class='widget-box'>
		...
		<h5>Tentang Pengarang</h5>
        </div>
		<hr>";
		while($data= mysqli_fetch_array($hasil)){
		echo
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahmengarang.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Mengarang :</label>
              <div class='controls'>
                <input type='text' name='id_mengarang' class='span11'  value='".$data['id_mengarang']."'/>
              </div>
            </div>
			<label class='control-label'>
			  ...
			  $sql2="SELECT * FROM buku";
				...
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_buku']."'>";
					echo $data2['judul_buku']; "</option>";
					} } 
					echo"
			</select>
			</div>
			<label class='control-label'>
			  ...
			  <option value=''>Pilih ID Pengarang</option>";
			  ...
			  $sql2="SELECT * FROM pengarang";
				...
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_pengarang']."'>";
					echo $data2['id_pengarang']; "</option>";
					} } 
					echo"
			</select>
			</div>
			...
			<label class='control-label'>Nama Pengarang :</label>
              <div class='controls'>
                <input type='text' name='nama_pengarang' class='span11'  value='".$data['nama_pengarang']."'/>
              </div>
            </div>
            <div class='form-actions'>
              <button class='btn btn-primary' name='kirim'>Ubah</button>
			  <a href='mengarang.php' class='btn btn-warning'> Kembali </a>
            </div>
          </form>";
		  }
