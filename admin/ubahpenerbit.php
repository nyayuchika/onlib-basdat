...
<div id="content">
  ...
  <?php
	...
	if(isset($_POST['kirim']))
	{
		...
		$alamat_penerbit=isset($_POST['alamat_penerbit']) ? $_POST['alamat_penerbit']:'';
		$sql="UPDATE penerbit SET id_penerbit='$id_penerbit', nama_penerbit='$nama_penerbit', alamat_penerbit='$alamat_penerbit' WHERE id_penerbit='$id_penerbit'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/penerbit.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/penerbit.php'; </script>";
		}
	}
	?>
	<?php
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahpenerbit.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Penerbit :</label>
              <div class='controls'>
                <input type='text' name='id_penerbit' class='span11'  value='".$data['id_penerbit']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Nama Penerbit :</label>
              <div class='controls'>
                <input type='text' name='nama_penerbit' class='span11'  value='".$data['nama_penerbit']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Alamat Penerbit :</label>
              <div class='controls'>
                <input type='text' name='alamat_penerbit' class='span11'  value='".$data['alamat_penerbit']."'/>
              </div>
            </div>
