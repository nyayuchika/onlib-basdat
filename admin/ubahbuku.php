...
<div id="content">
  ...
  <?php
	...
	if(isset($_POST['kirim']))
	{
		...
		$stok_buku=isset($_POST['stok_buku']) ? $_POST['stok_buku']:'';
		...
		$sql="UPDATE buku SET id_buku='$id_buku', id_kelompok='$id_kelompok', id_penerbit='$id_penerbit', id_tingkat='$id_tingkat', judul_buku='$judul_buku', isbn='$isbn', thn_terbit='$thn_terbit', stok_buku='$stok_buku', foto='$foto' WHERE id_buku='$id_buku'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/buku.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/buku.php'; </script>";
		}
	}
	?>
	<?php
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahbuku.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Buku :</label>
              <div class='controls'>
                <input type='text' name='id_buku' class='span11'  value='".$data['id_buku']."'/>
              </div>
            </div>
			<label class='control-label'>
			  ...
			  $sql2="SELECT * FROM kelompok_buku";
				$hasil2=mysqli_query($koneksi, $sql2);
				if (mysqli_num_rows($hasil2) > 0) {
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_kelompok']."'>";
					echo $data2['nama_kelompok']; "</option>";
					} } 
					echo"
			</select>
			</div>
			<label class='control-label'>
			  ...
			  $sql2="SELECT * FROM penerbit";
				...
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_penerbit']."'>";
					echo $data2['nama_penerbit']; "</option>";
					} } 
					echo"
			</select>
			</div>
			<label class='control-label'>
			  ...
			  $sql2="SELECT * FROM tingkat";
				$hasil2=mysqli_query($koneksi, $sql2);
				if (mysqli_num_rows($hasil2) > 0) {
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_tingkat']."'>";
					echo $data2['nama_tingkat']; "</option>";
					} } 
					echo"
			</select>
			</div>
			...
			<label class='control-label'>Judul :</label>
              <div class='controls'>
                <input type='text' name='judul_buku' class='span11'  value='".$data['judul_buku']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>ISBN :</label>
              <div class='controls'>
                <input type='text' name='isbn' class='span11'  value='".$data['isbn']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Tahun Terbit :</label>
              <div class='controls'>
                <input type='text' name='thn_terbit' class='span11'  value='".$data['thn_terbit']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Stok Buku :</label>
              <div class='controls'>
                <input type='text' name='stok_buku' class='span11'  value='".$data['stok_buku']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Foto :</label>
              <div class='controls'>
                <input type='file' name='foto' id='foto' class='span11'  value='".$data['foto']."'/>
              </div>
            </div>
