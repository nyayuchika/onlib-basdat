...
<div id="sidebar"><a href="admin.php" class="visible-phone"><i class="icon icon-home"></i>Dashboard</a>
  <ul>
	...
	<li class="submenu"> <a href="#"><i class="icon icon-th-list"></i> <span>Cakupan</span> <span class="label label-important">5</span></a>
      <ul>
        <li class="active"><a href="lantai.php"><span>Lantai</span></a> </li>
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="admin.php" title="Go to Dashboard" class="tip-bottom"><i class="icon-home"></i>Dashboard</a> <a href="lantai.php" class="current">Lantai</a> </div>
    <h1>Ubah</h1>
  </div>
  <?php
	include "config.php";
	if(isset($_POST['kirim']))
	{
		...
		$nama_lantai=isset($_POST['nama_lantai']) ? $_POST['nama_lantai']:'';
		$sql="UPDATE lantai SET id_lantai='$id_lantai', nama_lantai='$nama_lantai' WHERE id_lantai='$id_lantai'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/lantai.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/lantai.php'; </script>";
		}
	}
	?>
	<?php
	$id_lantai=isset($_GET['id_lantai']) ? $_GET['id_lantai']:'';
	$sql="SELECT * FROM lantai WHERE id_lantai='$id_lantai'";
	$hasil=mysqli_query($koneksi, $sql);
    echo "
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahlantai.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Lantai :</label>
              <div class='controls'>
                <input type='text' name='id_lantai' class='span11'  value='".$data['id_lantai']."'/>
              </div>
            </div>
			<div class='control-group'>
			<label class='control-label'>Nama Lantai :</label>
              <div class='controls'>
                <input type='text' name='nama_lantai' class='span11'  value='".$data['nama_lantai']."'/>
              </div>
            </div>
            <div class='form-actions'>
              <button class='btn btn-primary' name='kirim'>Ubah</button>
			  <a href='lantai.php' class='btn btn-warning'> Kembali </a>
            </div>
          </form>";
		  }
