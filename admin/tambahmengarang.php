        ...
        <div class="widget-box">
          ...
          <form method="post" class="form-horizontal">
			<label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM buku";
				...
				while ($data = mysqli_fetch_array($hasil)){?>
					...
					<?php echo $data['judul_buku'];?></option>
					<?php } } ?>
			</select>
			</div>
			<label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM pengarang";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_pengarang'];?>">
					<?php echo $data['id_pengarang'];?></option>
					<?php } } ?>
			</select>
			</div>
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="nama_pengarang" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO mengarang VALUES('$_POST[id_buku]','$_POST[id_pengarang]','$_POST[nama_pengarang]')");
				if($sql)
				{
					echo "<script>alert('Data berhasil ditambahkan'); window.location = 'mengarang.php'</script>";
				}
				else
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahmengarang.php'</script>";
				}
				}
			?>
          </form>
