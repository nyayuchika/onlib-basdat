...
<div id="sidebar"><a href="admin.php" class="visible-phone"><i class="icon icon-home"></i>Dashboard</a>
  <ul>
	...
	<li class="active"><a href="peminjaman.php"><span>Peminjaman</span></a></li>
...
<div id="content">
  ...
  <?php
	...
	if(isset($_POST['kirim']))
	{
		...
		$no_kartu=isset($_POST['no_kartu']) ? $_POST['no_kartu']:'';
		...
		$sql="UPDATE peminjaman SET no_pinjam='$no_pinjam', no_kartu='$no_kartu', id_buku='$id_buku', jumlah_pinjam='$jumlah_pinjam', denda='$denda' WHERE no_pinjam='$no_pinjam'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/peminjaman.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/peminjaman.php'; </script>";
		}
	}
	?>
	<?php
	$no_pinjam=isset($_GET['no_pinjam']) ? $_GET['no_pinjam']:'';
	$sql="SELECT * FROM peminjaman WHERE no_pinjam='$no_pinjam'";
	$hasil=mysqli_query($koneksi, $sql);
    echo "
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahpeminjaman.php' enctype='multipart/form-data'>
			...
			<div class='control-group'>
              <div class='controls'>
                <input type='hidden' name='no_kartu' class='span11'  value='".$data['no_kartu']."'/>
              </div>
			<div class='control-group'>
              <div class='controls'>
                <input type='hidden' name='id_buku' class='span11'  value='".$data['id_buku']."'/>
              </div>
			<div class='control-group'>
			<label class='control-label'>Jumlah Pinjam :</label>
              <div class='controls'>
                <input type='text' name='jumlah_pinjam' class='span11'  value='".$data['jumlah_pinjam']."'/>
              </div>
            </div>
			<div class='control-group'>
              <div class='controls'>
                <input type='hidden' name='denda' class='span11'  value='".$data['denda']."'/>
              </div>
            </div>
            <div class='form-actions'>
              <button class='btn btn-primary' name='kirim'>Ubah</button>
			  <a href='peminjaman.php' class='btn btn-warning'> Kembali </a>
            </div>
          </form>";
		  }
