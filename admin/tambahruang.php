        ...
        <div class="widget-content nopadding">
          <form method="post" class="form-horizontal">
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="id_ruang" class="span11"/>
              </div>
            </div>
            <label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM lantai";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_lantai'];?>">
					<?php echo $data['nama_lantai'];?></option>
					<?php } } ?>
			</select>
			</div>
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="nama_ruang" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO ruang(id_ruang,id_lantai,nama_ruang) VALUES('$_POST[id_ruang]','$_POST[id_lantai]','$_POST[nama_ruang]')");
				if($sql)
				{
					echo "<script>alert('Data berhasil ditambahkan'); window.location = 'ruang.php'</script>";
				}
				else
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahruang.php'</script>";
				}
				}
			?>
          </form>
