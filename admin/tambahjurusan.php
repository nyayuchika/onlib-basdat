        ...
        <div class="widget-content nopadding">
          <form method="post" class="form-horizontal">
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="id_jurusan" class="span11"/>
              </div>
            </div>
            <label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM fakultas";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_fakultas'];?>">
					<?php echo $data['nama_fakultas'];?></option>
					<?php } } ?>
			</select>
			</div>
			<div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="nama_jurusan" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO jurusan(id_jurusan,id_fakultas,nama_jurusan) VALUES('$_POST[id_jurusan]','$_POST[id_fakultas]','$_POST[nama_jurusan]')");
				if($sql)
				{
					echo "<script>alert('Data berhasil ditambahkan'); window.location = 'jurusan.php'</script>";
				}
				else
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahjurusan.php'</script>";
				}
				}
			?>
          </form>
