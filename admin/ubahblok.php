  ...
  <ul>
	...
	<li class="submenu"> <a href="#"><i class="icon icon-th-list"></i> <span>Cakupan</span> <span class="label label-important">5</span></a>
      <ul>
		...
		<li class="active"><a href="blok.php"><span>Blok</span></a></li>
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="admin.php" title="Go to Dashboard" class="tip-bottom"><i class="icon-home"></i>Dashboard</a> <a href="blok.php" class="current">Blok</a> </div>
    <h1>Ubah</h1>
  </div>
  <?php
	include "config.php";
	if(isset($_POST['kirim']))
	{
		...
		$nama_blok=isset($_POST['nama_blok']) ? $_POST['nama_blok']:'';
		$sql="UPDATE blok SET id_blok='$id_blok', id_ruang='$id_ruang', nama_blok='$nama_blok' WHERE id_blok='$id_blok'";
		$hasil = mysqli_query($koneksi, $sql);
		if($hasil) 
		{
			echo "<script>alert('Data Berhasil Diupdate'); document.location.href='http://localhost/chika/admin/blok.php'; </script>";
		} 
		else 
		{
			echo "<script>alert('Proses Gagal'); document.location.href='http://localhost/chika/admin/blok.php'; </script>";
		}
	}
	?>
	<?php
	$id_blok=isset($_GET['id_blok']) ? $_GET['id_blok']:'';
	$sql="SELECT * FROM blok WHERE id_blok='$id_blok'";
	$hasil=mysqli_query($koneksi, $sql);
    echo "
    ...
    <div class='span12'>
      <div class='widget-box'>
        ...
        "<div class='widget-content nopadding'>
          <form method='post' class='form-horizontal' action='ubahblok.php' enctype='multipart/form-data'>
			<div class='control-group'>
			<label class='control-label'>ID Blok :</label>
              <div class='controls'>
                <input type='text' name='id_blok' class='span11'  value='".$data['id_blok']."'/>
              </div>
            </div>
			<label class='control-label'>
			  ...
			  $sql2="SELECT * FROM ruang";
				$hasil2=mysqli_query($koneksi, $sql2);
				if (mysqli_num_rows($hasil2) > 0) {
				while ($data2= mysqli_fetch_array($hasil2)){
					echo"
					<option value='".$data2['id_ruang']."'>";
					echo $data2['nama_ruang']; "</option>";
					} } 
					echo"
			</select>
			</div>
			<div class='control-group'>
			<label class='control-label'>Nama Blok :</label>
              <div class='controls'>
                <input type='text' name='nama_blok' class='span11'  value='".$data['nama_blok']."'/>
              </div>
            </div>
            <div class='form-actions'>
              <button class='btn btn-primary' name='kirim'>Ubah</button>
			  <a href='blok.php' class='btn btn-warning'> Kembali </a>
            </div>
          </form>";
		  }
