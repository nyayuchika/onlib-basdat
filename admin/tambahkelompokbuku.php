        ...
        <div class="widget-box">
          ...
          <form method="post" class="form-horizontal">
			<div class="control-group">
              <label class="control-label">ID Kelompok :</label>
              <div class="controls">
                <input type="text" name="id_kelompok" class="span11"/>
              </div>
            </div>
			<label class="control-label">
			  ...
			  <?php $sql="SELECT * FROM jenis_buku";
				$hasil=mysqli_query($koneksi, $sql);
				if (mysqli_num_rows($hasil) > 0) {
				while ($data = mysqli_fetch_array($hasil)){?>
					<option value="<?php echo $data['id_jenis'];?>">
					<?php echo $data['nama_jenis'];?></option>
					<?php } } ?>
			</select>
			</div>
            <div class="control-group">
              ...
              <div class="controls">
                <input type="text" name="nama_kelompok" class="span11"/>
              </div>
            </div>
			...
			<?php
				...
				{
				$sql=$koneksi->query("INSERT INTO kelompok_buku VALUES('$_POST[id_kelompok]','$_POST[id_jenis]','$_POST[nama_kelompok]')");
				if($sql)
				{
					echo "<script>alert('Data berhasil ditambahkan'); window.location = 'kelompokbuku.php'</script>";
				}
				else
				{
					echo "<script>alert('Proses gagal'); window.location = 'tambahkelompokbuku.php'</script>";
				}
				}
			?>
          </form>
